...
Mean absolute error between `sim` and `obs`, in the same units of them,
...
\$\$ mae = \frac{1}{N} \sum\_{i=1}^N { \left\|S_i - O_i) \right\| } \$\$
...
the mean absolute error between each column of `sim` and `obs`.
