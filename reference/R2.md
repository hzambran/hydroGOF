...
product-moment correlation coefficient between simulated and observed
values.

It ranges from 0 to 1 when the correlation is defined. A value of 1
indicates a perfect positive or negative linear relationship between
`sim` and `obs`, while a value of 0 indicates no linear relationship.
